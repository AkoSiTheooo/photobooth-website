...
2. **Environment** (`.env.local`)
   ...
   | `UPLOAD_IP_SALT`                       | Any long random string, used to hash visitor IPs for rate limiting |

3. **Supabase project**
   - The migration in `supabase/migrations/` creates the tables, the row level
...
4. **Frames.** Drop a transparent image into `public/frames/` and add its entry to
   `public/frames/manifest.json`. The dev-only tool at `/dev/measure-frame` finds the
...
| `npm run lint`                     | ESLint                                                                                    |
| `node scripts/verify-supabase.mjs` | Checks the public Supabase surface: what a visitor can and cannot do, plus the rate limit |
...
- `proxy.ts`: session refresh and the admin gate
